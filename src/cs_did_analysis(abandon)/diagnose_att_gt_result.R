...
# diagnose_att_gt_result.R
# 用于诊断att_gt_result对象的结构和属性的脚本
# 帮助解决"invalid 'type' (list) of argument"错误
...
# 检查是否有输入参数 (可能是RDS文件路径)
args <- commandArgs(trailingOnly = TRUE)
rds_file <- if (length(args) > 0) args[1] else "att_gt_result.rds"

cat("==================================================\n")
cat("诊断att_gt_result对象\n")
cat("==================================================\n")
...
tryCatch({
  if (file.exists(rds_file)) {
    att_gt_result <- readRDS(rds_file)
    cat("成功读取att_gt_result对象\n")
  } else {
    stop("找不到RDS文件:", rds_file)
  }
}, error = function(e) {
  cat("错误:", e$message, "\n")
  quit(status = 1)
...
# 打印对象的基本信息
cat("\n对象基本信息:\n")
cat("对象类型:", class(att_gt_result), "\n")
cat("对象大小:", format(object.size(att_gt_result), units = "auto"), "\n\n")
...
# 打印关键组件的细节
key_components <- c("group", "t", "att", "DIDparams")
for (comp in key_components) {
  if (comp %in% names(att_gt_result)) {
    cat(comp, "组件:\n")
    if (comp == "DIDparams") {
      str(att_gt_result[[comp]], max.level = 1)
    } else {
      str(att_gt_result[[comp]])
    }
    cat("\n")
  }
}
...
# 尝试调用 aggte 并捕获错误信息
cat("尝试执行 aggte 函数的不同方法:\n")

# 方法1: 使用默认参数
cat("方法1: 默认参数\n")
tryCatch({
  agg_result1 <- did::aggte(att_gt_result)
  cat("成功! 结果类型:", class(agg_result1), "\n")
  cat("结构:\n")
  str(agg_result1, max.level = 1)
}, error = function(e) {
  cat("失败:", e$message, "\n")
})
cat("\n")
...
# 方法2: 添加 na.rm = TRUE
cat("方法2: 添加 na.rm = TRUE\n")
tryCatch({
  agg_result2 <- did::aggte(att_gt_result, na.rm = TRUE)
  cat("成功! 结果类型:", class(agg_result2), "\n")
  cat("结构:\n")
  str(agg_result2, max.level = 1)
}, error = function(e) {
  cat("失败:", e$message, "\n")
})
cat("\n")
...
cat("方法3: 使用字符串type参数\n")
tryCatch({
  agg_result3 <- did::aggte(att_gt_result, type = "dynamic", na.rm = TRUE)
  cat("成功! 结果类型:", class(agg_result3), "\n")
  cat("结构:\n")
  str(agg_result3, max.level = 1)
}, error = function(e) {
  cat("失败:", e$message, "\n")
})
cat("\n")
...
cat("方法4: 使用简单type参数\n")
tryCatch({
  agg_result4 <- did::aggte(att_gt_result, type = "group", na.rm = TRUE)
  cat("成功! 结果类型:", class(agg_result4), "\n")
  cat("结构:\n")
  str(agg_result4, max.level = 1)
}, error = function(e) {
  cat("失败:", e$message, "\n")
})
cat("\n")

# 检查att_gt_result的详细结构
cat("检查att_gt_result的type参数相关结构:\n")
if ("DIDparams" %in% names(att_gt_result)) {
  params <- att_gt_result$DIDparams
  cat("DIDparams:\n")
  str(params, max.level = 2)
  
  # 特别检查可能与type相关的参数
  relevant_params <- c("type", "anticipation", "control_group", "panel")
  for (param in relevant_params) {
    if (param %in% names(params)) {
      cat("\nDIDparams$", param, ":\n", sep="")
      print(params[[param]])
      cat("类型:", class(params[[param]]), "\n")
    }
  }
}
...
cat("解决方案建议:\n")
cat("==================================================\n")
cat("1. 如果上述方法中有成功的，请使用对应的代码替换失败的部分\n")
cat("2. 如果都失败，考虑手动构建动态处理效应数据:\n")
cat("   - 提取att_gt_result中的处理组、时间和处理效应信息\n")
cat("   - 计算相对于处理时间的期数\n")
cat("   - 手动聚合处理效应\n")
cat("   - 构建自己的动态处理效应图表\n\n")
